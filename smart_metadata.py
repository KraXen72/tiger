...
def get_most_likely_tag(list_of_keys, obj, additional_values = []):
	...
	sorted_counts = list(sorted(counts_entries, key = lambda x: x[1])) # sort it (ascending)
	...
	if len(dehashed_counts) > 1 and dehashed_counts[0][1] == dehashed_counts[1][1]:
		...
		if type(top_result) == type("string") and type(second_result) == type({}):
...
def smart_metadata(info):
	...
	# additional values we want to consdier when picking most likely tag
	add_values = { 
		"title": [],
		"artist": [],
		"album_artist": [],
	...
	# here the results of get_most_likely_tag will be dumped
	others = add_values.copy()
